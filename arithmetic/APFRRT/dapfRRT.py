...
class dapfRRT:
    ...
    def collision(self, src, dst):
        """
            ...
            """
        if dst[0] <= 0 or dst[0] >= self.width or dst[1] <= 0 or dst[1] >= self.height or src[0] <= 0 or src[1] <= 0 or src[1]>=self.width or src[1]>=self.height:
            return True
        if (self.collFlag):
            self.collFlag = False
            return False
    ...
    def plan(self, plan_surface):
        ...
        for i in range(max_iterations):
            ...
            if current_node:
                ...
                continue
            # 2. 检查是否达到目标点
            if new_node and self.is_goal_reached(new_node, (self.end.x, self.end.y), max_distance*3):
                ...
                for k in range(len(smoothed_path) - 1):
                    pygame.draw.line(
                        plan_surface,
                        (255, 0, 0),  # 路径颜色
                        (smoothed_path[k].x, smoothed_path[k].y),  # 当前点
                        (smoothed_path[k + 1].x, smoothed_path[k + 1].y),  # 下一点
                        2  # 线宽
                    )
                QApplication.processEvents()
                path.reverse()
                ...
                print("总花费时间为")
                print(end - start)
                return smoothed_path, middtimer - start
